...
class VisionTransformerVPT(nn.Module):
    ...
    def __init__(self, model_name='vit_large_patch16_384', img_size=384, patch_size=16, in_chans=3, embed_dim=1024, depth=24,
        ...
        val = math.sqrt(6. / float(3 * reduce(mul, [self.patch_size, self.patch_size], 1) + self.embed_dim))  # noqa
        self.prompt_embeddings = nn.Parameter(torch.zeros(1, self.num_tokens, self.embed_dim))
        nn.init.uniform_(self.prompt_embeddings.data, -val, val)
        #
        # print(int(sum([np.prod(p.shape) for p in self.prompt_proj.parameters()])))
        # self.prompt = prompt
        self.prompt = 'vpt_deep'
        if 'deep' in self.prompt:
            total_d_layer = self.depth
            self.prompt_embeddings = nn.Parameter(torch.zeros(
                total_d_layer, self.num_tokens, self.embed_dim))
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            x = torch.cat((
                        cls_tokens,
                        self.prompt_dropout(self.prompt_proj(self.prompt_embeddings[i]).expand(B, -1, -1)),
                        feat
            ), dim=1)
            x = blk(x)
            cls_tokens = x[:, :1, :]
            feat = x[:, 1+self.num_tokens:, :]
            if i in self.out_indices:
                outs.append(torch.cat([cls_tokens, feat], dim=1))
        # cls_tokens = x[:, :1, :]
        # feat = x[:, 1:, :]
        # for i in range(self.depth):
        #     x = torch.cat((
